...
type PnLCalculationServiceTests() =
    ...
    member this.``Calculate Total PnL with Only Losses``() =
        let transactions = [
            { TransactionType = Sell; BuyPrice = 150m; SellPrice = 100m; Amount = 1m; TransactionDate = DateTime(2021, 1, 1) }
        ]
        let result = calculateTotalPnL transactions
    ...
    member this.``Calculate Total PnL with Break-Even Transactions``() =
        let transactions = [
            { TransactionType = Buy; BuyPrice = 100m; SellPrice = 100m; Amount = 1m; TransactionDate = DateTime(2021, 1, 1) }
        ]
        let result = calculateTotalPnL transactions
    ...
    member this.``Calculate Total PnL with High Precision Prices``() =
        let transactions = [
            { TransactionType = Buy; BuyPrice = 100.1234m; SellPrice = 100.5678m; Amount = 1.2345m; TransactionDate = DateTime(2021, 1, 1) }
        ]
        let result = calculateTotalPnL transactions
    ...
    member this.``Calculate Historical PnL on Boundary Dates``() =
        let transactions = [
            { TransactionType = Buy; BuyPrice = 100m; SellPrice = 200m; Amount = 1m; TransactionDate = DateTime(2021, 1, 1) };
            { TransactionType = Sell; BuyPrice = 200m; SellPrice = 150m; Amount = 1m; TransactionDate = DateTime(2021, 12, 31) }
        ]
    ...
    member this.``Historical PnL Calculation with Future Transactions``() =
        let transactions = [
            { TransactionType = Buy; BuyPrice = 100m; SellPrice = 200m; Amount = 1m; TransactionDate = DateTime(2022, 1, 1) }
        ]
        let dateRange = { StartDate = DateTime(2021, 1, 1); EndDate = DateTime(2021, 12, 31) }
        let result = calculateHistoricalPnL transactions dateRange
        match result with
    ...
    member this.``Historical PnL for Short Date Range``() =
        let transactions = [
            { TransactionType = Buy; BuyPrice = 150m; SellPrice = 300m; Amount = 2m; TransactionDate = DateTime(2021, 6, 15) }
        ]
